...
$Header: /nfs/slac/g/glast/ground/cvs/calibGenCAL/src/lib/Specs/CalGeom.cxx,v 1.5 2008/04/18 21:46:26 chehtman Exp $
*/
...
namespace calibGenCAL {
  ...
  namespace CalGeom {
    CalUtil::XtalIdx pos2Xtal(const Vec3D &pos) {
      ...
      const float twrRow0ctrY = CU_GEOM ? 0 : -1.5;

      // tower 'columns' numbered in X dimension
      const short twrCol = (short)(pos.x()/twrPitch + 2);


      if (twrCol < 0 || twrCol > 3) return INVALID_XTAL;

      // tower 'rows' numbered in Y dimension
      const short twrRow = (short)(pos.y()/twrPitch + 0.5-twrRow0ctrY);
      if (twrRow < 0 || twrRow > 3) return INVALID_XTAL;
      const TwrNum twr(twrRow,
                       twrCol);

      const float zDiff  = CalTopZ - pos.z();
      const LyrNum lyr( (short)(zDiff / cellVertPitch));
      ...
      const float  zOffset =  zDiff-lyr.val()*cellVertPitch;
      if ( zOffset > CsIHeight)
        return INVALID_XTAL;

      ColNum col;
      if (lyr.getDir() == X_DIR) {
        const float twrCtrY = (twrRow+twrRow0ctrY)*twrPitch;
        col = (short)((pos.y() - twrCtrY)/cellHorPitch + 6);

        // check that we are not in xtal gap
        const float xtalCtr = cellHorPitch*((float)col.val()-5.5)+twrCtrY;
        if (abs(pos.y() - xtalCtr) > CsIWidth/2)
          return INVALID_XTAL;
      } else {
        // Y_DIR
        const float twrCtrX = (twrCol-1.5)*twrPitch;
        col = (short)((pos.x() - twrCtrX)/cellHorPitch + 6);
        // check that we are not in xtal gap
        const float xtalCtr = cellHorPitch*((float)col.val()-5.5)+twrCtrX;
