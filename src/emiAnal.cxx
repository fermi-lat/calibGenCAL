...
int main(int argc,
  ...
  TH1F         *hcallo     = new TH1F("hcallo", "CAL_LO rate per time slice", n_time_slices, 0, n_time_slices*
                                      time_slice);
  TH1F         *hcalhi     = new TH1F("hcalhi", "CAL_HI rate per time slice", n_time_slices, 0, n_time_slices*
                                      time_slice);
  ...
  for (int twr = 0; twr < 16; twr++) {
    ...
    pr_ped_twr[twr] = new TProfile(prpedtwrname.str().c_str(), prpedtwrname.str().c_str(), n_time_slices, 0, ticksmax
                                   -ticks0, -10, 10);
    for (int xy = 0; xy < 2; xy++)
      for (int face = 0; face < 2; face++) {
        ostringstream prpedafeename;
        prpedafeename <<  "prpedafee_T" << twr << "_" << ( (xy == 0) ? "X" : "Y") << ( (face == 0) ? "P" : "N");
        pr_ped_afee[twr][xy][face] = new TProfile(prpedafeename.str().c_str(), prpedafeename.str().c_str(),
                                                  n_time_slices, 0, ticksmax-ticks0, -10, 10);
      }
  }
    ...
    for (int lyr = 0; lyr < 8; lyr++)
      for (int face = 0; face < 2; face++)
        for (int col = 0; col < 12; col++) {
          ...
          h_ped_chan[twr][lyr][col][face]  = new TH1F(hpedname.str().c_str(), hpedname.str().c_str(), 200, -100,
                                                      100);
          pr_ped_chan[twr][lyr][col][face] = new TProfile(prpedname.str().c_str(), prpedname.str().c_str(),
                                                          n_time_slices, 0, ticksmax-ticks0, -50, 50);
        }
