...
- <b> Light Asymmetry </b> Light asymmetry vs position represented by
- 10 points along each crystal length.

- <b> Integral Nonlinearity </b> adc2dac conversion in all 4 ranges.
  Corrects for nonlinearity in adc scale.

- \b MevPerDAC  Relationship of total energy deposited to the
  ...
  than ADC scale.
- \b Thresholds  Measurement of FLE & FHE triggers, ULD & LAC
- thresholds in ADC units.
...
-  \b runMuonCalib.exe  Reads data from one or more muon collection
...
- \b CalDefs.h Collection of classes used for contiguous indexing of
...
- \b RootFileAnalysis This class provides generic root file playback
...
- \b CGCUtil.h contains generic, non-GLAST related functions which are
- shared throughout the calibGenCAL package. (string, stream & vector
- manipulation, etc...).
...
configuration file specifies the input data sets and tower 
...
of the CAL for each calibration desired.  A set of characterization
files produced
...
of the various CAL calibration

XML file types.  The checks are usually nothing more than limit and
consitency checks.  Warnings or

errors generated may be benign.  The '-R' produces plots and
...
characterization data XML files:
	...
	* Perform linear extrapolation for FLE LEX8 characterization
	* data; needed for higher FLE thresholds
	* Digital smoothing filter as final processing
This tool can be used as a pre-processor for other tools which require
clean characterization tables
...
The four scripts get_slac_calibdac.py, build_acdsmooth.py,
build_gensettings_cfg.py and gensettings.py automate the process of
generating DAC settings using the genXXXsettings.py
(genFLEsettings.py, genFHEsiettings.py, genLACsettings.py and genULDsettings.py)
scripts. They are meant to operate in the LAT I&T environment, in that they assume run numbers and file names as generated by the I&T version of the calibDAC suite and subsequent pipeline analyses.
