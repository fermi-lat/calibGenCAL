...
This package contains codes to generate and study calibration constants for CAL using muon and charge injection data. The package generates the following calibrations for use in offline software.

- Pedestals  Noise pedestal position and width are measured.
- <b> Light Asymmetry </b> Light asymmetry vs position represented by 10 points along each crystal length.
- <b> Integral Nonlinearity </b> adc2dac conversion in all 4 ranges.  Corrects for nonlinearity in adc scale.
- \b MevPerDAC  Relationship of total energy deposited to the geometric mean of the DAC values at both ends of the xtal.  Onboard DAC scale units are used for calculation b/c it is much more linear than ADC scale.
- \b Thresholds  Measurement of FLE & FHE triggers, ULD & LAC thresholds in ADC units.
 
\b Applications:
-  \b runMuTrigEff.exe
-  \b runCIFit.exe  Generates spline function points for adc2dac conversions. Reads digi root files containing charge injection calibration data. Expected test configuration is described in code.   Reads config in from ciFit_option.xml, use ciFit_option_badCalibGen.xml if charge injection test used wrong calib_gain setting.
-  \b runMuonCalib.exe  Reads data from one or more muon collection digi root files and produces calibration constant files in XML format (pedestals,asymetry and MevPerDAC). It also produces root histogram files for studying the quality of the calibration constants. The executable takes input from muonCalib_option.xml, use muonCalib_option_badCalibGen.xml for CI tests w/ wrong calib_gain setting.

\b Shared Classes & Headers:
- \b CalDefs.h Collection of classes used for contiguous indexing of different cal components. Supports numerous conversions, incremen, and field get/set() routines.  Also contains useful Cal constant definitions.
- \b RootFileAnalysis This class provides generic root file playback for Glast ROOT format event data.  This functionality was extracted from RootAnalysis/RootFileAnalysis.cxx. It allows the user to treat an arbitrary collection of digi/mc/recon GLAST root data as a collection of events which may be stepped through/'seeked'/rewound, etc...  Intended to be inherited by application specific class that provides the data structures, event loop & output code.
- \b CGCUtil.h contains generic, non-GLAST related functions which are shared throughout the calibGenCAL package. (string, stream & vector manipulation, etc...).
...
- \b biasVal      [-V] [-E <err_limit>] [-W <warn_limit>] [-R <root_file>] [-L <log_file>] <xml_file>
- \b intNonlinVal [-V] [-E <err_limit>] [-W <warn_limit>] [-R <root_file>] [-L <log_file>] <xml_file>
- \b mevPerDacVal [-V] [-E <err_limit>] [-W <warn_limit>] [-R <root_file>] [-L <log_file>] <xml_file>
