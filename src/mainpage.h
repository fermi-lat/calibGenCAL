...
launch most of the python tools in either Windows (.bat) or Linux (.sh)
...
example cfg is in python/cfg/runSuiteParallel.cfg
command line usage can be obtained w/ runSuiteParallel -h.
...
- <tt> dacVal       [-V] [-E <err_limit>] [-W <warn_limit>] [-R <root_file>] [-L <log_file>]
...
- dacDiff.py
dacDiff.py will calculate per-channel difference between any two cal dac 
settings files.  This can be used for trending & validation.
...
of the calibDAC suite and subsequent pipeline analyses.

see calibGenCAL/doc/gensettings_scripts.html for more information.

<b> generic utilit scripts </b>
...
the unit_test subfolder contains cfg files & validated output for most of the calibGenCAL applications.
It selects 2 towers from an 8 tower data run for speed purposes, but still testing  multi-tower
functionality.

unit_test/output contains the validated output.

the unit test runs the following apps
- runSuiteParallel
- ciFit
- muonCalib
- muTrig
