...
__author__    = "D.L.Wood"
__date__      = "$Date: 2005/05/05 14:32:17 $"
...
if __name__ == '__main__':
    ...
    for opt in options:
        ...
        if destTwr < 0 or destTwr > 15:
            log.error("pedMerge: dest index for [infiles] option %s out of range (0 - 15)", opt)
            sys.exit(1)
        ...
        if srcTwr < 0 or srcTwr > 15:
            log.error("pedMerge: src index for [infiles] option %s out of range (0 - 15)", opt)
            sys.exit(1)
